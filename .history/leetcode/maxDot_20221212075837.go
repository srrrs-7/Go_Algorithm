...
func maxDotProduct(nums1 []int, nums2 []int) int {
	...
	for i := 1; i < len(nums1); i++ {
		dp[i][0] = max(dp[i-1][0], nums1[i]*nums2[0]) // nums1 compare max
	}
	for i := 1; i < len(nums2); i++ {
		dp[0][i] = max(dp[0][i-1], nums1[0]*nums2[i]) // nums2 compare max
	}
	for i := 1; i < len(nums1); i++ {
		for j := 1; j < len(nums2); j++ {
			// non-empty subsequences, so dp[i-1][j-1] may be negative, I WA here :(
